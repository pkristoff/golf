    ...
    context 'Average strokes & putts' do
      ...
      it 'Returns avg strokes & putts round for two round' do
        ...
        expected_stroke_avg.each_with_index do |expected_strokes, index|
          expect(pc_average_holes[index].strokes).to eq(expected_strokes), "strokes mismatch for #{index}: pc_average_holes[index].strokes: #{pc_average_holes[index].strokes} does not eq strokes: #{expected_strokes}"
        end
