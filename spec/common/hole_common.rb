...
module HoleCommon
  ...
  def expect_edit_hole(page_or_rendered, tee, values)
    ...
    expect_holes_list(page_or_rendered, tee, values)
  ...
  def expect_edit_hole_fields(page_or_rendered, is_disabled, values)
    ...
    if page_or_rendered.is_a? String
      ...
      expect(page_or_rendered).to have_selector("fieldset input[id=hole_number][disabled=disabled][value=#{values[:number]}]",
                                                count: 1)
      ...
      expect(find_field(Label::Hole::NUMBER, disabled: true).value).to eq(values[:number])
      expect(find_field(Label::Hole::YARDAGE, disabled: is_disabled).value).to eq(values[:yardage])
      expect(find_field(Label::Hole::PAR, disabled: is_disabled).value).to eq(values[:par])
      expect(find_field(Label::Hole::HDCP, disabled: is_disabled).value).to eq(values[:hdcp])
    end
  end
