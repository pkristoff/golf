...
describe Round, type: :model do
  ...
  describe 'add_score' do
    it 'add a score for hole' do
    end

    it 'error if score for hole number already exists' do
    ...
    it 'error if score for hole of another tee' do
      ...
      round1 = described_class.new(tee: tee1)
    ...
    it 'error if score for hole of another course' do
      ...
      hole2 = tee2.sorted_holes.second
      round1 = described_class.new(tee: hole1.tee)
      round1.add_score(hole1, 5, 5, '')
      # rubocop:disable Layout/LineLength
  ...
  describe 'self.rounds' do
    it 'return an empty array if none is found' do
      course = Course.create(name: 'foo')
      expect(described_class.rounds(course)).to be_empty
    end

    it 'return an array of Round if found' do
      course = round.course
      rounds = described_class.rounds(course)
  ...
  describe 'destroy' do
    it 'remove Round and scores used by Round, but not tee, course' do
  ...
  describe 'next_score' do
    it 'return the score for hole two given score for hole 1' do
      ssh = round.sorted_score_holes
      expect(round.next_score(ssh[0].score)).to eq(ssh[1].score)
    end

    it 'return the score for hole ten given score for hole 9' do
      ssh = round.sorted_score_holes
      expect(round.next_score(ssh[8].score)).to eq(ssh[9].score)
    end

    it 'return the score for hole eighteen given score for hole 9' do
      ssh = round.sorted_score_holes
